...
def run_match(bot, map):
    ...
    try:
        outputA = str(subprocess.check_output(['./gradlew', '--debug', 'run', '-PteamA=' + currentBot, '-PteamB=' + bot, '-Pmaps="' + map + '"'], shell=True))
        outputB = str(subprocess.check_output(['./gradlew', '--debug', 'run', '-PteamA=' + bot, '-PteamB=' + currentBot, '-Pmaps="' + map + '"'], shell=True))
