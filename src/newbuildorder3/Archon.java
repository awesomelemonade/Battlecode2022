...
public class Archon implements RunnableBot {
    ...
    public void init() throws GameActionException {
        wantedEarlygameMiners = Math.min(8, Math.max(5, MAP_WIDTH * MAP_HEIGHT / 140)) / rc.getArchonCount();
    }
    ...
    public void tryBuildRatio(int builder, int miner, int attacker) throws GameActionException {
        int prod = (builder <= 0 ? 1 : builder) * (miner <= 0 ? 1 : miner) * (attacker <= 0 ? 1 : attacker);
        int builderScore = builder <= 0 ? Integer.MAX_VALUE : Communication.getAliveRobotTypeCount(RobotType.BUILDER) * (prod/builder);
        int minerScore = miner <= 0 ? Integer.MAX_VALUE : Communication.getAliveRobotTypeCount(RobotType.MINER) * (prod/miner);
        int attackerScore = attacker <= 0 ? Integer.MAX_VALUE : (Communication.getAliveRobotTypeCount(RobotType.SOLDIER) + Communication.getAliveRobotTypeCount(RobotType.SAGE) + Communication.getAliveRobotTypeCount(RobotType.WATCHTOWER)) * (prod / attacker);
        if (minerScore < attackerScore && minerScore < builderScore) {
